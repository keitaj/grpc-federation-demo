...
// versions:
//
//	protoc-gen-grpc-federation: v1.27.0
//
...
func NewBFFService(cfg BFFServiceConfig) (*BFFService, error) {
	...
	celEnvOpts = append(celEnvOpts, grpcfed.GRPCErrorAccessorOptions(celTypeHelper, "order.GetOrderResponse")...)
	celEnvOpts = append(celEnvOpts, grpcfed.GRPCErrorAccessorOptions(celTypeHelper, "order.ListOrdersByUserResponse")...)
	celEnvOpts = append(celEnvOpts, grpcfed.GRPCErrorAccessorOptions(celTypeHelper, "product.GetProductResponse")...)
	celEnvOpts = append(celEnvOpts, grpcfed.GRPCErrorAccessorOptions(celTypeHelper, "product.ListProductsResponse")...)
...
func (s *BFFService) resolve_Bff_UserStatistics(ctx context.Context, req *BFFService_Bff_UserStatisticsArgument) (*UserStatistics, error) {
	...
	// (grpc.federation.field).by = "$.orders.reduce(accum, cur, accum + cur.total_amount, 0.0)"
	if err := grpcfed.SetCELValue(ctx, &grpcfed.SetCELValueParam[float64]{
		Value:      value,
		Expr:       `$.orders.reduce(accum, cur, accum + cur.total_amount, 0.0)`,
		CacheIndex: 146,
		Setter: func(v float64) error {
			ret.TotalSpent = v
			return nil
